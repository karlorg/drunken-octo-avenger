...
(function() {
  var $pointAt, game_common, game_marking, getEmptyRegions, go_rules, markStonesAround, reviveAroundRegion, setInitialDead, setRegionScores, setupScoring, tesuji_charm, togglePoints, updateForm,
  ...
  setInitialDead = function() {
    "mark dead stones on the DOM according to the supplied value of the hidden input field dead_stones";
    var $point, color, dead_stones, e, k, len, ref, x, y;
    try {
    ...
    for (k = 0, len = dead_stones.length; k < len; k++) {
      ref = dead_stones[k], x = ref[0], y = ref[1];
      $point = $pointAt(x, y);
  ...
  updateForm = function() {
    ...
    var dead_stones, k, l, len, len1, point, row, state, x, y;
    dead_stones = [];
    state = game_common.readBoardState();
    for (y = k = 0, len = state.length; k < len; y = ++k) {
      row = state[y];
      for (x = l = 0, len1 = row.length; l < len1; x = ++l) {
        point = row[x];
