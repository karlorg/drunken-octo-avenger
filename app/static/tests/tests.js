...
(function() {
  var $pointAt, go_rules, imageSrc, isPointBlack, isPointBlackDead, isPointBlackScore, isPointEmpty, isPointWhite, isPointWhiteDead, isPointWhiteScore, updateBoardChars;
  ...
  updateBoardChars = function(charArray) {
    var char, col, color, row, rowString, _i, _j, _len, _len1;
    for (row = _i = 0, _len = charArray.length; _i < _len; row = ++_i) {
      rowString = charArray[row];
      for (col = _j = 0, _len1 = rowString.length; _j < _len1; col = ++_j) {
        char = rowString[col];
        color = (function() {
          switch (char) {
            case "b":
            ...
            case ".":
              return "empty";
            default:
              throw new Error("unknown board char: " + char);
          }
        })();
        tesuji_charm.game_common.setPointColor(tesuji_charm.game_common.$pointAt(col, row), color);
      }
    }
  };
